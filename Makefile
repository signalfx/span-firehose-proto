...
generate:
	mkdir -p gen
	docker run --rm -v $(PWD):$(PWD) -w $(PWD) znly/protoc --gofast_out=./gen/ -I./ -I./vendor/github.com/gogo/protobuf/ -I./vendor/ kafka.proto
...
.PHONY: goimports
goimports:
	@IMPORTSOUT=`$(GOIMPORTS) -local github.com/signalfx/span-firehose-proto -d $(ALL_SRC) 2>&1`; \
	if [ "$$IMPORTSOUT" ]; then \
...
.PHONY: impi
impi:
	@$(IMPI) --local github.com/signalfx/span-firehose-proto --scheme stdThirdPartyLocal $(ALL_SRC)
